...
{
  /* HOST->ZW: mode | funcID */   // funcID is optional, host 
  /* ZW->HOST: retVal */
  ...
  if (SERIALPI_SET_LEARN_MODE_LEARN_PLUS_OFFSET > frame->payload[0]) {
    ...
    EQueueNotifyingStatus queueStatus = QueueNotifyingSendToBack(ZAF_getZwCommandQueue(), (uint8_t *)&Command, 0);
    retVal = (EQUEUENOTIFYING_STATUS_SUCCESS == queueStatus) ? true : false;
    if (retVal) {
      ZPAL_LOG_DEBUG(ZPAL_LOG_APP, "ZW_SET_LEARN_MODE queued, mode=0x%02x useCB=%u\n", frame->payload[0], frame->payload[1] != 0);
    } else {
      ZPAL_LOG_WARNING(ZPAL_LOG_APP, "ZW_SET_LEARN_MODE queue failed, mode=0x%02x useCB=%u\n", frame->payload[0], frame->payload[1] != 0);
    }
  } else {
    ...
    if (retVal) {
      ZPAL_LOG_DEBUG(ZPAL_LOG_APP, "ZW_SET_LEARN_MODE (NM) started, mode=0x%02x result=0x%02x\n", frame->payload[0], retVal);
    } else {
      ZPAL_LOG_WARNING(ZPAL_LOG_APP, "ZW_SET_LEARN_MODE (NM) failed to start, mode=0x%02x\n", frame->payload[0]);
    }
  }
  DoRespond(retVal);
}
