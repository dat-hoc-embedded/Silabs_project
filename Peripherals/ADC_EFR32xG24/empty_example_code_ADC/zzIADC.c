...
void initIADC (void)
{
  ...
  IADC_InitSingle  _t initSingle = IADC_INITSINGLE_DEFAULT;
...
static void cli_afe_init(sl_cli_command_arg_t *args)
{
  (void)args;
  sl_status_t st = sl_analog_front_end_init(NULL);
  app_log_info("afe: init -> 0x%04lX\n", (unsigned long)st);
}

static const sl_cli_command_info_t cmd_afe_init = 
  SL_CLI_COMMAND(cli_afe_init, "Initialize IADC analog front-end (defaults)", "", {SL_CLI_ARG_END, });

static sl_cli_command_entry_t afe_cmd_table[] = {
  {"init", &cmd_afe_init, false},
  {"start", &cmd_afe_start, false},
  {NULL, NULL, false},
};


const sl_cli_command_info_t efuse_cli_analog_front_end_cmd_group = 
  SL_CLI_COMMAND_GROUP(afe_cmd_table, "IADC analog front-end commands");


static sl_cli_command_entry_t efuse_cli_cmd_table[] = {
  {"afe", &efuse_cli_analog_front_end_cmd_group, false},
  {NULL, NULL, false}
};

static sl_cli_command_group_t efuse_cli_cmd_group = {
  {NULL},
  false,
  efuse_cli_cmd_table,
};